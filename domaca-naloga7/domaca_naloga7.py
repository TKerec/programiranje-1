...
def mnozica_vescin(s):
    ...
    for vescina in s.split():
        vescine.add(pomen_vescin[vescina])
    return vescine

def dvosmerni_zemljevid(zemljevi):
    obojesmerni = defaultdict()
    for k,v in zemljevi.items():
        obojesmerni[(k[0],k[1])] = obojesmerni[(k[1],k[0])] = mnozica_vescin(v)
    return obojesmerni

def mozna_pot(pot, zemljevid):
    obojesmerni = dvosmerni_zemljevid(zemljevid)
    for i in range(len(pot)-1):
        if (pot[i], pot[i+1]) not in obojesmerni:
            return False
    return True
...
def potrebne_vescine(pot, zemljevid):
    ...
    vescine = set()
    for i in range(len(pot)-1):
        vescine |= obojesmerni[(pot[i], pot[i+1])]
    return vescine
